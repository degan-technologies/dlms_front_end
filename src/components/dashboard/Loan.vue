...
<template>
    <div class="card p-4 shadow mb-10 rounded-lg bg-white">
        ...
        <!-- Data Table -->
        <DataTable v-if="!loading && !error && categories.length" :value="categories" :rows="perPage" :paginator="false" responsiveLayout="scroll">
        ...
        <!-- Empty State -->
        <div v-else-if="!loading && !categories.length && !error" class="text-gray-500">No categories found.</div>
        <!-- Pagination Controls -->
        <div class="flex justify-center items-center gap-2 mt-4">
            <button
                class="px-3 py-1 rounded border transition font-semibold hover:bg-green-600 hover:text-white"
                :class="currentPage === 1 ? 'bg-gray-200 text-gray-400' : 'bg-green-500 text-white'"
            ...
            <button
                class="px-3 py-1 rounded border transition font-semibold hover:bg-green-600 hover:text-white"
                :class="currentPage === totalPages ? 'bg-gray-200 text-gray-400' : 'bg-green-500 text-white'"
