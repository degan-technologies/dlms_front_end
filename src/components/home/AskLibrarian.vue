      ...
      <section v-if="isOpen" class="mt-4 w-full max-w-md sm:max-w-lg md:max-w-xl bg-white rounded-xl shadow-xl border border-indigo-100 flex flex-col">
        ...
        <div ref="chatBox" class="flex-1 overflow-y-auto p-4 space-y-4 bg-indigo-50 min-h-[40vh] max-h-[60vh]">
          <div v-for="(pair, index) in messages" :key="index" class="space-y-3">
            ...
            <div class="flex justify-end italic text-gray-500" v-else>
              <div class="max-w-[75%] px-4 py-2 rounded-lg text-sm shadow bg-indigo-100 text-indigo-700">
                No answer yet.
              </div>
            </div>
          </div>
        ...
        <form @submit.prevent="sendMessage" class="flex flex-wrap items-center gap-2 p-4 border-t border-indigo-100 bg-white rounded-b-xl">
          ...
          <button
            ...
            class="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 disabled:bg-indigo-300 disabled:cursor-not-allowed"
          >
            Send
          </button>
        </form>
