...
async function markAsRead(id) {
    try {
        await axiosInstance.put(`/admin/notifications/${id}/mark-as-read`);
...
async function markAllAsRead() {
    try {
        await axiosInstance.put('/admin/notifications/{id}/mark-all-read');
...
async function clearAllNotifications() {
    try {
        await axiosInstance.delete('/admin/notifications/{id}/clear-all');
        notifications.value = [];
        unreadCount.value = 0;
    } catch (err) {
...
<template>
    <div class="p-4 border border-gray-200 rounded-lg sm:rounded-xl sm:p-6 relative">
        <!-- Header -->
        <div class="flex items-center justify-between mb-4 sm:mb-6">
            ...
            <div v-if="notifications.length > 0">
                <Button icon="pi pi-ellipsis-v" class="p-button-text p-button-plain p-button-rounded !w-8 !h-8 sm:!w-10 sm:!h-10" @click="menu.toggle($event)" />
        ...
        <div v-else ref="notificationsContainer" class="">
            <div v-for="(group, date) in groupedNotifications" :key="date">
                ...
                <ul class="p-0 mx-0 mt-0 mb-4 sm:mb-6 list-none space-y-2 sm:space-y-0">
                    <li
            ...
            <template #footer>
                <div class="flex justify-between">
                    ...
                    <div class="flex gap-2">
                        ...
                        <Button label="View Loan" icon="pi pi-external-link" class="p-button-primary" @click="$router.push(selectedNotification.actions.view_loan)" v-if="selectedNotification?.actions?.view_loan" />
                    </div>
                </div>
            </template>
        </Dialog>
